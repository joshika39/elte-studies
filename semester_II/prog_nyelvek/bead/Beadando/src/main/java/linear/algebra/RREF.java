...
public class RREF {
    ...
    public static double[][] rref(double[][] matrix){
        ...
        for(int k = 0; k < numRows; k++){
            ...
            if(matrix[k][lead] != 0){
                matrix = rowScale(matrix, k, ((double) 1/matrix[k][lead]));
            }
            for(i = 0; i < numRows; i++){
                if(i != k){
                    matrix = rowAddScale(matrix, k, i,((double)(-1) * matrix[i][lead]));
                }
            }
            lead++;
        }
    ...
    private static double[][] rowAdd(double[][] matrix, int rowIndex1,
        ...
        for(int k = 0; k < numColumns; k++){
            matrix[rowIndex2][k] += (double)matrix[rowIndex1][k];
        }
    ...
    private static double[][] rowAddScale(double[][] matrix, int rowIndex1,
        ...
        for(int k = 0; k < numColumns; k++){
            matrix[rowIndex2][k] += ((double)matrix[rowIndex1][k] * scalar);
        }
